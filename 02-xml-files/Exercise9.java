...
public class Exercise9 {
	...
	public static void addDirector(Document doc, String title, String directorName,String directorSurname) {
		Element nodoPelicula = doc.createElement("pelicula");
		NodeList movies;

		NodeList titles = doc.getDocumentElement().getElementsByTagName("titulo");
		
		for (int i = 0; i < titles.getLength(); i++) {
			if(titles.item(i).getFirstChild().getNodeValue().equals(title)){
				movies=titles.item(i).getParentNode().getChildNodes();
			}
		}
